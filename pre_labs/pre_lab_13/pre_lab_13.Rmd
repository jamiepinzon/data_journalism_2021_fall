...
**Answer** This data frame is showing whether Trump or Clinton won that state on the lection, and the percentage of the businesses recieving loans from the states. 
```{r}
...
**Answer**There is a positive correlation between states with more trump support and businesses receiving ppp loans.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer**The correlation is still positive with the average amount of money provided by PPP per small business employee in the state and Trump support.But it is not as strong. 
```{r}
...
**Answer** the R value of the two different graphs before. 
```{r}
reveal_data %>%
...
**Answer** the  r value is 0.5218038. The P value is 8.607e-05.It is very low, so we can say that there is a stronger correlation between the two. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** the r value is 0.221133 which isn't that strong. The P value is 0.1189, it is a bit high so it's worse than the previous. 
```{r}
...
**Answer** The table is showing the mean ppp loan amount per employee and the mean ppp laons that the businesses receive from Clinton states and Trump states. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** The mean for both groups are the same as the table above. The T value is -1.2223, and the p value is 0.2295.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** The mean is still the same from the table above. The t value is higher, -3.182 and the p value is 0.002643.
```{r}
